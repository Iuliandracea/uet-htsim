...
DragonFlyPlusTopology::DragonFlyPlusTopology(uint32_t p, uint32_t l, uint32_t s, uint32_t h, linkspeed_bps linkspeed, mem_b queuesize,
...
DragonFlyPlusTopology::DragonFlyPlusTopology(uint32_t radix, linkspeed_bps linkspeed, mem_b queuesize, QueueLoggerFactory* logger_factory,
                                                EventList* ev, queue_type q, simtime_picosec latency, simtime_picosec switch_latency, topology_type type) {
    _queuesize = queuesize;
    _logger_factory = logger_factory;
    _eventlist = ev;
    _linkspeed = linkspeed;
    ...
    for (uint32_t j = 0; j < _no_of_spines; j++) {
        ...
        if (_type == DFP_DENSE_T) {
            for (uint32_t target_group_id = 0; target_group_id < _no_of_groups; target_group_id++) {
                ...
                uint32_t larger = groupid >= target_group_id;
            ...
            for (uint32_t l = 0; l < _h; l++) {
                uint32_t targetgroupid = (j % _s) * _h + l;
                ...
                if (larger)
                    ...
                    continue;
                uint32_t k  = targetgroupid * _s + groupid / _h;
